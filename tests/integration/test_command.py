...
def stop(
    ...
    queues: list[queue_proxy_wrapper.QueueProxyWrapper],
) -> None:
    """
    Stop the workers.
    """
...
def put_queue(
    queue: queue_proxy_wrapper.QueueProxyWrapper, msgs: list[telemetry.TelemetryData]
) -> None:
    """
    Place mocked inputs into the input queue periodically with period TELEMETRY_PERIOD.
    """
    for msg in msgs:
        queue.queue.put(msg, timeout=TELEMETRY_PERIOD)
...
def main() -> int:
    ...
    manager = mp.Manager()
    # Create your queues
    response_queue = queue_proxy_wrapper.QueueProxyWrapper(manager)
    ...
    threading.Timer(
        TELEMETRY_PERIOD * len(path), stop, (controller, [data_queue, response_queue])
    ).start()
    ...
    command_worker.command_worker(connection, TARGET, data_queue, response_queue, controller)
    # =============================================================================================
    #                          ↑ BOOTCAMPERS MODIFY ABOVE THIS COMMENT ↑
    # =============================================================================================
